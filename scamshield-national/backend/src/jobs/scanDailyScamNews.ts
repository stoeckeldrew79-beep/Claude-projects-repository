...
import { US_SEARCH_TERMS, INTERNATIONAL_SEARCH_TERMS } from './dailyNewsSearchTerms';
...
//
// Every term was measured against the original five before being added:
// each one returns stories the others do not. Roughly 2.5x the unique
// headlines of the original set. International coverage is done with
// country-targeted terms, not Google News' locale parameters — setting
// hl/gl/ceid to GB, AU, CA, IE, SG, IN or NZ was measured and does NOT
// meaningfully change the results for an English query — US and GB returned
// 85 of the same 87 links, and four locales together yielded 95 unique links
// against 87 for US alone. Naming the country's own fraud agency in the
// query is what actually surfaces local coverage. The two lists live in
// dailyNewsSearchTerms.ts, shared with the dailyNews controller, which needs
// to tell a US story from an international one after the fact.
